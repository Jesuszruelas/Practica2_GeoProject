# Practica2_GeoProject
Practica 2 Web
## Guía Práctica: Geolocalización con OSM y ASP.NET Core

**Objetivo:** Desarrollar una aplicación web MVC que consuma APIs geográficas gratuitas (Nominatim y Overpass) para ubicar al usuario en un mapa y encontrar puntos de interés cercanos sin costo.

#1 - Geolocalización
Jesus Daniel Osuna Luna
Gidel Zabdiel Jimenez Cervantes
Daniel Omar Almada Serrano
Jesús René Zamora Ruelas
Jesus Dario Taveres Hernandez